...
def test_static_content_is_rendered_immediately() -> None:
    renderer = ContentRenderer(ScreenContent.static("first\nsecond"))
...
def test_dynamic_content_is_replaced_only_when_result_is_delivered() -> None:
    renderer = ContentRenderer(ScreenContent.dynamic(lambda: "unused"))
...
def test_streamed_content_changes_only_when_chunks_are_appended() -> None:
    renderer = ContentRenderer(ScreenContent.stream(iter(["unused"])))
...
def test_content_renderer_requires_screen_content() -> None:
    with pytest.raises(TypeError, match="ScreenContent"):
        ContentRenderer(["valid", 42])  # type: ignore[arg-type]
...
def test_content_dimensions_use_visible_width_and_safe_ansi() -> None:
    renderer = ContentRenderer(
        ScreenContent.static("\x1b[31m界\x1b[0m\x1b[2J\ne\u0301")
    )
